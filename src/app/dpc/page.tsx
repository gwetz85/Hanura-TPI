...
export default async function DpcPage() {
  ...
  if (!session || !["DPC", "ADMIN", "PETUGAS"].includes(session.user?.role || "")) redirect("/login");
  ...
  const [ktaCount, activityCount, pacUsers, memberCounts, maleCount, femaleCount, totalMembers] = await Promise.all([
    ...
    prisma.user.findMany({ where: { role: { notIn: ["DPC", "ADMIN", "PETUGAS"] } }, select: { id: true, name: true, role: true } }),
